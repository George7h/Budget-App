...
RSpec.describe "expenses/edit", type: :feature do
  before(:each) do
    ...
    user.groups.create!(name: 'Electricity', icon: File.open(Rails.root.join('app', 'assets', 'images', 'defaultgroupicon.png')))
    user.groups.create!(name: 'Junk food', icon: File.open(Rails.root.join('app', 'assets', 'images', 'defaultgroupicon.png')))
  end
